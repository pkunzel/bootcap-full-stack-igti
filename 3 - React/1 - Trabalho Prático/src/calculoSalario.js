...
function calculoSalario(fullSalary) {
  const baseINSS = +fullSalary;
    document.querySelector("#inputBaseINSS").value = format(baseINSS);

  const discountINSS = calculateDiscountINSS(baseINSS);
  document.querySelector("#inputDescINSS").value = format(discountINSS);

  const baseIRPF = baseINSS - discountINSS;
  document.querySelector("#inputBaseIRPF").value = format(baseIRPF);

  const discountIRPF = calculateDiscountIRPF(baseIRPF);
  document.querySelector("#inputDescIRPF").value = format(discountIRPF);

  const netSalary = baseINSS - discountINSS - discountIRPF;
  document.querySelector("#inputValLiquido").value = format(netSalary);

  console.log(netSalary/fullSalary);
}
