import gradesDao from "../Database/gradesDao.js";

class gradesModel {
	constructor() {
		this._gradesDao = new gradesDao();
	}
	...
	*/
	insertRecord(newRecord) {
		this._gradesDao.insertRecord(newRecord);
		return { success: true };
	}
	...
	*/
	updateRecord(updatedRecord) {
		this._gradesDao.updateRecord(updatedRecord);
		return { success: true };
	}
	...
	*/
	deleteRecord(recordId) {
		this._gradesDao.deleteRecord(recordId);
		return { success: true };
	}
	...
	*/
	getRecord(id) {
		return this._gradesDao.getRecord(id);
	}
    ...
    */
	getNotaTotalAluno(student, subject) {
		return { nota : this._gradesDao
			.getAllMatches({ student, subject })
    ...
    */
	getMediaNotaSubject(subject, type) {
        const registros = this._gradesDao.getAllMatches({ subject, type });
		const total = registros.reduce((acc, cur) => ({ value: acc.value + cur.value })).value;
		return { media: total / registros.length };
	}
    ...
    */
	getMelhoresGrades(subject, type) {
		const registros = this._gradesDao
			.getAllMatches({ subject, type })
            .sort((a, b) => b.value - a.value)
            .map(x => ({ id : x.id, value : x.value}));

		// if (registros.length > 3) {
		// 	return registros.slice(0, 3);
		// }
